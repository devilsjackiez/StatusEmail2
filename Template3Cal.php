...
// Query status OPEN
$sql = "SELECT `template_version`,count(`action`)FROM sugarcrm_hr.email_tracking WHERE template_version='Zenith2016V3' && `action`='open'
...
// Query status Unread
$sql = "SELECT `template_version`,count(`action`)FROM sugarcrm_hr.email_tracking WHERE template_version='Zenith2016V3' && `action`='unread'
        AND (date_modified <= DATE_FORMAT(UTC_TIMESTAMP(),'%Y-%m-%d') AND date_modified  >= DATE_SUB(DATE_FORMAT(UTC_TIMESTAMP(),'%Y-%m-%d'),INTERVAL 6 MONTH))";
$result = $conn->query($sql);
...
$sql = "SELECT `template_version`,count(`action`)FROM sugarcrm_hr.email_tracking WHERE template_version='Zenith2016V3' && `action`='replied'
        AND (date_modified <= DATE_FORMAT(UTC_TIMESTAMP(),'%Y-%m-%d') AND date_modified  >= DATE_SUB(DATE_FORMAT(UTC_TIMESTAMP(),'%Y-%m-%d'),INTERVAL 6 MONTH))";
$result = $conn->query($sql);
...
// Query status Click
$sql = "SELECT `template_version`,count(`action`)FROM sugarcrm_hr.email_tracking WHERE template_version='Zenith2016V3' && `action`='click'
        AND (date_modified <= DATE_FORMAT(UTC_TIMESTAMP(),'%Y-%m-%d') AND date_modified  >= DATE_SUB(DATE_FORMAT(UTC_TIMESTAMP(),'%Y-%m-%d'),INTERVAL 6 MONTH))";
